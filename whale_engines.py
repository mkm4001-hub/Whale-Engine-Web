...
class DataEngine:
    ...
    def load_market(self, target_code, latest_stock_date):
        ...
        if mkt_ticker == "^TWOII":
            ...
            if mkt is None or mkt.empty or not isinstance(mkt.index, pd.DatetimeIndex) or mkt_latest < latest_stock_date:
        ...
        if not mkt.empty:
            if stock_latest == mkt_latest:
                ...
                if len(common_idx) > 0:
                    ...
                    if days_behind <= 5: sync_msg = f" (啟用多日比對: 大盤延遲{days_behind}天)"
                    else: market_status, sync_msg = "Stale", " (大盤資料過舊)"
                else: sync_msg = " (無共同交易日)"
    ...
    def calculate(self, data, custom_params=None):
        ...
        try:
            ...
            if (is_volume_high and is_price_stagnant) or memory_triggered:
                ...
                else: retreat_checks.extend([(f"{vol_str}(底部換手不扣分)", False if is_inst_complete else "Unknown"), ("爆量不漲(底部良性換手)", False)])
    ...
    def calculate(self, data, mode):
        ...
        if is_complete:
            ...
            if len(df_valid_trust) >= 5:
                ...
                if trust_consecutive_buy >= 3 or trust_5d_sum > (vol_avg_5d * 0.1) or trust_5d_sum > 1000:
                ...
                elif trust_5d_sum < 0 and (trust_consecutive_sell >= 2 or trust_5d_sum < -500):
                    score -= 15
                    ...
                    messages.append(f"【投信微幅調節】單日小賣 {abs(int(trust_buy))} 張，但近5日累計仍買超 {int(trust_5d_sum)} 張，持續觀察。")
...
class ChipXRayEngine:
    def calculate(self, tdcc_df, fish_score, retreat_score):
        if tdcc_df is None or tdcc_df.empty or len(tdcc_df) < 2:
            ...
            return {
                "xray_status": "快取累積中",
                "xray_message": f"本地快取庫僅有 {avail} 週記錄，需累積滿 2 週啟動突襲偵測，3 週啟動趨勢偵測",
                "is_surge": False
            }
